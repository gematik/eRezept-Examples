...
find "$DIR" -type f -name "*.xml" -print0 | while IFS= read -r -d '' file; do
    # Create a temporary file
    temp_file=$(mktemp)
    
    # Apply the sed commands and save to temporary file
    sed -e 's/<meta id="[^"]*"/<meta/g' \
        -e 's/000\.000\.01-\([0-9]\{1,2\}\)/000.000.01/g' \
        "$file" > "$temp_file"
